...
public: // publid 이라는 가시성이 지시됨. 이제부터 나열되는 멤버들은 외부에서도 자유롭게 사용할 수 있다.
    ...
    // 완전한 복사를 만들기 위한 새로운 복사 생성자를 만듦! 명시적 복사 생성자!
    VecF(const VecF& fv ) : n { fv.n } { // 복사 생성자이니까 같은 클래스의 객체를 참조받는 생성자 VecF& fv 를 만든다.
    // VecF 클래스의 참조인 fv 를 매개변수로 받아서 그대로 복사하게 복사 생성자를 명시적으로 만들어보는 것이다.
    // n { fv.n } <= VecF 클래스 객체의 n 의 값은 fv 의 n 값을 그대로 복사한다. n 의 값을 fv 의 n 으로 초기화

        arr = new float[n]; // arr 은 new 명령어로 새로운 데이터를 저장할 메모리를 할당받아 가리킨다.
        // new 명령어로 새로 생성했기 때문에 이 "arr = new float[n]" 에서의 arr 과 "const VecF& fv" 에서의 fv 가 가리키고 있는 arr 은 완전히 별개의 메모리가 된다.

        memcpy(arr, fv.arr, sizeof(float) * n); // 데이터를 복사한다.
        // memcpy 명령어를 이용해 fv의 arr 에 들어있는 내용을 arr 에 복사해라.
        // 이 때 데이터를 복사할 바이트 수는 float 의 길이 sizeof(float) * 데이터의 개수 n 만큼 복사를 해라.
    }
